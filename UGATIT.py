...
from ops import *
from tensorflow.contrib.data import (map_and_batch, prefetch_to_device,
                                     shuffle_and_repeat)
from utils import *
...
class UGATIT(object):
    def __init__(self, sess, args):
        ...
        self.trainA_dataset = glob(
            './dataset/{}/*.*'.format(self.dataset_name + '/trainA'))
        self.trainB_dataset = glob(
            './dataset/{}/*.*'.format(self.dataset_name + '/trainB'))
        self.dataset_num = max(len(self.trainA_dataset),
                               len(self.trainB_dataset))
    ...
    def generator(self, x_init, reuse=False, scope="generator"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            x = conv(x_init,
                     channel, kernel=7, stride=1, pad=3,
                     pad_type='reflect',
                     scope='conv')
            ...
            cam_gmp_logit, cam_x_weight = fully_connected_with_w(cam_x,
                                                                 reuse=True,
                                                                 scope='CAM_logit')
            x_gmp = tf.multiply(x, cam_x_weight)

            cam_logit = tf.concat([cam_gap_logit, cam_gmp_logit],
                                  axis=-1)
            x = tf.concat([x_gap, x_gmp],
                          axis=-1)

            x = conv(x,
                     channel, kernel=1, stride=1,
                     scope='conv_1x1')
            x = relu(x)
            ...
            for i in range(2):
                ...
                x = conv(x,
                         channel//2, kernel=3, stride=1, pad=1,
                         pad_type='reflect',
                ...
                channel = channel // 2
            x = conv(x,
                     channels=3, kernel=7, stride=1, pad=3,
                     pad_type='reflect',
                     scope='G_logit')
            x = tanh(x)
    ...
    def discriminator(self, x_init, reuse=False, scope="discriminator"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            local_x, local_cam, local_heatmap = self.discriminator_local(x_init,
                                                                         reuse=reuse,
                                                                         scope='local')
            global_x, global_cam, global_heatmap = self.discriminator_global(x_init,
                                                                             reuse=reuse,
                                                                             scope='global')
    ...
    def discriminator_global(self, x_init, reuse=False, scope='discriminator_global'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            x = conv(x_init,
                     channel, kernel=4, stride=2, pad=1,
                     pad_type='reflect',
                     sn=self.sn,
                     scope='conv_0')
            x = lrelu(x, 0.2)
            ...
            for i in range(1, self.n_dis - 1):
                x = conv(x,
                         channel * 2, kernel=4, stride=2, pad=1,
                         pad_type='reflect',
                         sn=self.sn,
                         scope='conv_' + str(i))
                x = lrelu(x, 0.2)
            ...
            x = conv(x,
                     channel * 2, kernel=4, stride=1, pad=1,
                     pad_type='reflect',
                     sn=self.sn,
                     scope='conv_last')
            x = lrelu(x, 0.2)
            ...
            heatmap = tf.squeeze(tf.reduce_sum(x, axis=-1))
            x = conv(x,
                     channels=1, kernel=4, stride=1, pad=1,
                     pad_type='reflect',
                     sn=self.sn,
                     scope='D_logit')
    ...
    def discriminate_real(self, x_A, x_B):
        real_A_logit, real_A_cam_logit, _, _ = self.discriminator(x_A,
                                                                  scope="discriminator_A")
        real_B_logit, real_B_cam_logit, _, _ = self.discriminator(x_B,
    ...
    def discriminate_fake(self, x_ba, x_ab):
        fake_A_logit, fake_A_cam_logit, _, _ = self.discriminator(x_ba,
                                                                  reuse=True,
                                                                  scope="discriminator_A")
        fake_B_logit, fake_B_cam_logit, _, _ = self.discriminator(x_ab,
                                                                  reuse=True,
                                                                  scope="discriminator_B")
    ...
    def gradient_panalty(self, real, fake, scope="discriminator_A"):
        ...
        for i in range(2):
            grad = tf.gradients(logit[i], interpolated)[
            ...
            if self.gan_type == 'wgan-lp':
                GP.append(
                    self.ld * tf.reduce_mean(tf.square(tf.maximum(0.0, grad_norm - 1.))))

            elif self.gan_type == 'wgan-gp' or self.gan_type == 'dragan':
                GP.append(self.ld * tf.reduce_mean(tf.square(grad_norm - 1.)))

        for i in range(2):
            grad = tf.gradients(cam_logit[i], interpolated)[
                0]  # gradient of D(interpolated)
            grad_norm = tf.norm(flatten(grad), axis=1)  # l2 norm
            ...
            if self.gan_type == 'wgan-lp':
                ...
                cam_GP.append(
                    self.ld * tf.reduce_mean(tf.square(grad_norm - 1.)))
    ...
    def build_model(self):
        if self.phase == 'train':
            ...
            trainA = trainA \
                ...
                .apply(map_and_batch(Image_Data_Class.image_processing,
                                     self.batch_size,
            ...
            x_aba, _ = self.generate_b2a(x_ab,
                                         reuse=True)  # real b
            x_bab, _ = self.generate_a2b(x_ba,
                                         reuse=True)  # real a
            ...
            # GX: Compute generator adversarial losses
            ...
            # GX: Compute discriminator adversarial losses
            D_ad_loss_A = (discriminator_loss(self.gan_type,
                                              real_A_logit,
                                              fake_A_logit)
                           + discriminator_loss(self.gan_type,
                                                real_A_cam_logit,
                                                fake_A_cam_logit)
                           + GP_A
                           + GP_CAM_A)
            D_ad_loss_B = (discriminator_loss(self.gan_type,
                                              real_B_logit,
                                              fake_B_logit)
                           + discriminator_loss(self.gan_type,
                                                real_B_cam_logit,
            ...
            # GX: Multiply losses by their weights
            ...
            Generator_A_loss = Generator_A_gan \
                + Generator_A_cycle \
                + Generator_A_identity \
                + Generator_A_cam
            Generator_B_loss = Generator_B_gan \
                + Generator_B_cycle \
            ...
            # GX: Add regularization loss
            self.Generator_loss = Generator_A_loss \
                + Generator_B_loss \
                + regularization_loss('generator')
            self.Discriminator_loss = Discriminator_A_loss \
            ...
            self.G_A_loss = tf.summary.scalar("G_A_loss",
                                              Generator_A_loss)
            self.G_A_gan = tf.summary.scalar("G_A_gan",
                                             Generator_A_gan)
            self.G_A_cycle = tf.summary.scalar("G_A_cycle",
                                               Generator_A_cycle)
            self.G_A_identity = tf.summary.scalar("G_A_identity",
                                                  Generator_A_identity)
            self.G_A_cam = tf.summary.scalar("G_A_cam",
                                             Generator_A_cam)

            self.G_B_loss = tf.summary.scalar("G_B_loss",
                                              Generator_B_loss)
            self.G_B_gan = tf.summary.scalar("G_B_gan",
                                             Generator_B_gan)
            self.G_B_cycle = tf.summary.scalar("G_B_cycle",
                                               Generator_B_cycle)
            self.G_B_identity = tf.summary.scalar("G_B_identity",
                                                  Generator_B_identity)
            self.G_B_cam = tf.summary.scalar("G_B_cam",
                                             Generator_B_cam)

            self.D_A_loss = tf.summary.scalar("D_A_loss",
                                              Discriminator_A_loss)
            self.D_B_loss = tf.summary.scalar("D_B_loss",
                                              Discriminator_B_loss)
            ...
            for var in tf.trainable_variables():
                if 'rho' in var.name:
                    ...
                    self.rho_var.append(tf.summary.scalar(var.name + "_min",
                                                          tf.reduce_min(var)))
                    self.rho_var.append(tf.summary.scalar(var.name + "_max",
                                                          tf.reduce_max(var)))
                    self.rho_var.append(tf.summary.scalar(var.name + "_mean",
                                                          tf.reduce_mean(var)))
            ...
            """ Test """
            self.test_domain_A = tf.placeholder(tf.float32,
                                                [
                                                    1,
                                                    self.img_size,
                                                    self.img_size,
                                                ...
                                                name='test_domain_A')
            self.test_domain_B = tf.placeholder(tf.float32,
                                                [
                                                    1,
                                                    self.img_size,
                                                    self.img_size,
    ...
    def train(self):
        ...
        # summary writer
        self.writer = tf.summary.FileWriter(self.log_dir + '/' + self.model_dir,
                                            self.sess.graph)
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                if (counter - 1) % self.n_critic == 0:
                    batch_A_images, batch_B_images, fake_A, fake_B, _, g_loss, summary_str = self.sess.run([
                        ...
                        self.G_optim,
                        self.Generator_loss, self.G_loss
                    ],
                        feed_dict=train_feed_dict)
                    self.writer.add_summary(summary_str, counter)
                    past_g_loss = g_loss
    ...
    def test(self):
        tf.global_variables_initializer().run()
        test_A_files = glob(
            './dataset/{}/*.*'.format(self.dataset_name + '/testA'))
        test_B_files = glob(
            './dataset/{}/*.*'.format(self.dataset_name + '/testB'))
        ...
        for sample_file in test_A_files:  # A -> B
            ...
            fake_img = self.sess.run(self.test_fake_B,
                                     feed_dict={
        ...
        for sample_file in test_B_files:  # B -> A
            ...
            sample_image = np.asarray(load_test_data(sample_file,
                                                     size=self.img_size))
            image_path = os.path.join(self.result_dir,
                                      '{0}'.format(os.path.basename(sample_file)))

            fake_img = self.sess.run(self.test_fake_A,
                                     feed_dict={
                                         self.test_domain_B: sample_image
                                     })
